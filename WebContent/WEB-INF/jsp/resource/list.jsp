    ...
    <script type="text/javascript">
        function add(){
            ...
            if (resourceName == "") {
                ...
                $("#resourceName-info").show();
            } else if(permission == ""){
                $("#resourceName-info").hide();
        ...
        function update(){
            var resourceName = $("#resourceNameU").val();
            var permission = $("#permissionU").val();
            if (resourceName == "") {
                $("#permissionU-info").hide();
                $("#resourceNameU-info").show();
            } else if(permission == ""){
                $("#resourceNameU-info").hide();
                $("#permissionU-info").show();
            }else{
        ...
        function showUpdate (id) {
            ...
            $.ajax({
                    ...
                    data : {id : id},
                    dataType : 'json',
                    url: '${pageContext.request.contextPath}/resource/showUpdate',
                    success : function(d){
                        if(1 == d.status){
                            $('#idU').val(id);
                            $('#resourceNameU').val(d.data.resourceName);
                            $('#resourceDescU').val(d.data.resourceDesc);
                            $('#urlU').val(d.data.url);
                            $('#permissionU').val(d.data.permission);
                            $("#editForm").modal();
                        }else{
                            alert('错误'+d.message)
                        }
        ...
        function del (id) {
            // body...
            if (confirm('真的要删除么？')) {
                window.location.href = "${pageContext.request.contextPath}/resource/"+id+"/delete";
                return true;
            } else{
                return false;
            };
       ...
       <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
        ...
        <div id="page-wrapper">
            ...
            <div class="container-fluid">
                                ...
                                <tbody>
                                    <c:forEach items="${resourceList}" var="resource">
										<tr>
											...
											<td>
                                                <button type="button" id="updateButton" class="btn btn-primary btn-xs" title="修改" ><i class="fa fa-pencil" onclick="showUpdate('${resource.id}')"></i></button>
                                                <shiro:hasPermission name="resource:action">
                                                <button type="button" class="btn btn-primary btn-xs" title="删除" onclick="del('${resource.id}')"><i class="fa fa-trash"></i></button>
                                                </shiro:hasPermission>
                                            </td>
										</tr>
									</c:forEach>
                ...
                <!-- /.row -->
                <shiro:hasPermission name="resource:action">
                <button type="button" class="btn btn-primary" id="addButton" style="float:right;width:10%">新增资源</button>
                </shiro:hasPermission>
            </div>
            <!-- /.container-fluid -->
        </div>
       ...
       <div class="modal-dialog">
          <div class="modal-content">
             ...
             <div class="modal-body">
                <form id="resourceAddForm" action="${pageContext.request.contextPath}/resource/add" method="post">
                <input type="hidden" name="id">
                <div class="modal-row">
                    <div class="modal-row-left">资源名称：</div>
                    <div class="modal-row-middle">
                        <input type="text" class="form-control" id="resourceName" name="resourceName">
                    </div>
                    <div class="modal-row-right" id="resourceName-info"><i class="fa fa-warning"></i>资源名称不能为空</div>
                </div>
                <div class="modal-row">
                    <div class="modal-row-left">资源描述：</div>
                    <div class="modal-row-middle">
                         <input type="text" class="form-control" name="resourceDesc" id="resourceDesc">
                    </div>
                    <div class="modal-row-right"></div>
                </div>
                <div class="modal-row">
                    <div class="modal-row-left">URL：</div>
                    <div class="modal-row-middle">
                ...
                <div class="modal-row">
                    ...
                    <div class="modal-row-right" id="permission-info"><i class="fa fa-warning"></i>权限不能为空</div>
                </div>
                </form>
             </div>
    ...
    <div class="modal fade" id="editForm" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
       <div class="modal-dialog">
          <div class="modal-content">
             ...
             <div class="modal-body">
                <form id="resourceEditForm" action="${pageContext.request.contextPath}/resource/update" method="post">
                <input type="hidden" name="id" id="idU">
                <div class="modal-row">
                    <div class="modal-row-left">资源名称：</div>
                    <div class="modal-row-middle">
                        <input type="text" class="form-control" id="resourceNameU" name="resourceName">
                    </div>
                    <div class="modal-row-right" id="resourceNameU-info"><i class="fa fa-warning"></i>资源名称不能为空</div>
                </div>
                <div class="modal-row">
                    <div class="modal-row-left">资源描述：</div>
                    <div class="modal-row-middle">
                         <input type="text" class="form-control" name="resourceDesc" id="resourceDescU">
                    </div>
                    <div class="modal-row-right"></div>
                </div>
                <div class="modal-row">
                    <div class="modal-row-left">URL：</div>
                    <div class="modal-row-middle">
                        <input type="text" class="form-control" name="url" id="urlU">
                    </div>
                    <div class="modal-row-right"></div>
                </div>
                <div class="modal-row">
                    <div class="modal-row-left">权限：</div>
                    <div class="modal-row-middle">
                        <input type="text" class="form-control" name="permission" id="permissionU">
                    </div>
                    <div class="modal-row-right" id="permissionU-info"><i class="fa fa-warning"></i>权限不能为空</div>
                </div>
                </form>
             </div>
             <div class="modal-footer">
